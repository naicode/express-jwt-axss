...
var DEFAULT_GET_ANTI_XSS_TOKEN_FUNCTION = function(req) {
  if (!req.cookies) throw new Error('cookie field missing');
...
module.exports = function(options) {
  ...
  var middleware = function(req, res, next) {
    ...
    if (getAntiXSSToken) {
      ...
      if (!antiXSSToken) {
        return next(new UnauthorizedError('xss_token_missing', { message: 'AntiXSSToken is missing' }));
      }
    }
    ...
    async.parallel([
      ...
      if (!getAntiXSSToken) {
        ...
        async.parallel([
          ...
          if (decoded.jti !== antiXSSDecoded.refJti) {
            return next(new UnauthorizedError('invalid_anti_xss_token_ref', {message: 'antiXSS token is not meant for this JWT'}));
          } else {
            req[_requestProperty] = decoded;
          }
          next();
